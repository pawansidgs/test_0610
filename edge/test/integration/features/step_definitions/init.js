...
var environment = config['test_0610'].env;
...
console.log('&test_0610 api: [' +  domains[environment] + ', ' + config['test_0610'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_0610'].app, config['test_0610'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_0610'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
